...
export function renderItems(items) {
  ...
  items.forEach((item) => {
    ...
    catalogItem.innerHTML = `
      <div class="item__group">
        <div class="item__favorite ${isFavorite}">
          <svg
            width="20"
            height="19"
            viewBox="0 0 20 19"
  ...
  // Делегирование событий на контейнер каталога
  catalogContainer.addEventListener("click", (event) => {
    // Проверяем, что клик был по иконке heart-outline
    const heartIcon = event.target.closest(".heart-outline");
    if (heartIcon) {
      // Тогглим класс на родителе item__favorite с использованием функции toggleFavorite
      const favoriteElement = heartIcon.closest(".item__favorite");
      toggleFavorite(favoriteElement); // Вызов функции для переключения класса
    }
  });
}
